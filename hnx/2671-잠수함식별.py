...
input_s = str(sys.stdin.readline())

n = len(input_s)

splitter = [i for i in range(n) if input_s[i] == "1"]
isSubmarine = False
last = -1 # 직전 신호 확인ㅇ용
...
for i in range(1, len(splitter)):
    # 1의 위치 기준으로 문자열 자르기, 앞 1 제외 뒤 1 포함
    s_slice = input_s[splitter[i-1]+1:splitter[i]+1]
    ...
    s_slice = input_s[splitter[i - 1]:splitter[i] + 1]
    ...
    if s_slice == '11' and (last == 1 or last == 11 or (input_s[splitter[i - 1] - 1:splitter[i] + 2] == '0110')):
...
# 특수한 경우 처리
if input_s[0] == '0' and (n == 1 or (n >= 2 and input_s[:2] != '01')):
    # 길이가 짧거나, 0으로 시작하는데 2번째 수가 1이 아닌 경우
    isSubmarine = False
elif input_s[0] == '1' and n < 4:
